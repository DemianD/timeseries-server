...
class RawData extends MultidimensionalInterface {
  ...
  async onData(data) {
    ...
    this.lastObserved = new Date(
      this.latestRDFStore.getQuads(null, 'http://www.w3.org/ns/sosa/resultTime')[0].object.value
    );
  ...
  setupPollInterfaces() {
    ...
    super.commMan.http.get(this.liveURI + this.name + '/fragments', async (ctx, next) => {
      ...
      let fragments = Utils.getAllFragments(this.fragmentsPath).map(f =>
        new Date(f.substring(0, f.indexOf('.json'))).getTime()
      );
      ...
      let responseObj = {
        ...
        '@id':
          this.hostName + this.liveURI + this.name + '/fragments?time=' + fragment.toISOString(),
        '@graph': []
      };

      let fc = Utils.getFragmentsCount(this.fragmentsPath);
      let content = (
        await Utils.getFileContent(this.fragmentsPath + '/' + fragment.toISOString() + '.json')
      ).split('\n');
      content.pop();
      content = content
        .map(c => JSON.parse(c))
      ...
      responseObj = this.addMetadata(
        this.hostName + this.liveURI + this.name + '/fragments',
        responseObj,
        fragments,
        index
      );
