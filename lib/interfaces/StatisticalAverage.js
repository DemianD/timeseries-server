import MultidimensionalInterface from '../MultidimensionalInterface.js';
...
class StatisticalAverage extends MultidimensionalInterface {
  ...
  setupPollInterfaces() {
    ...
    super.commMan.router.get('/' + this.name + '/fragment/:year/:month', async (ctx, next) => {
      ...
      let filePath =
        this.fragmentsPath +
        '/' +
        ctx.params.year.split('_')[0] +
        '-' +
        ctx.params.month.split('_')[0] +
        '.trig';
      await this.handleRequest(ctx, filePath, true);
    });
    ...
    super.commMan.router.get('/' + this.name + '/fragment/:year/:month/:day', async (ctx, next) => {
      ...
      let filePath =
        ...
        ctx.params.month.split('_')[0] +
        '-' +
        ctx.params.day.split('_')[0] +
        '.trig';

      await this.handleRequest(ctx, filePath, true);
    });

    super.commMan.router.get(
      '/' + this.name + '/fragment/:year/:month/:day/:hour',
      async (ctx, next) => {
        ctx.response.set({ 'Access-Control-Allow-Origin': '*' });
        let filePath =
          this.fragmentsPath +
  ...
  async handleRequest(ctx, filePath, metadata) {
    if (!Utils.exists(filePath)) {
      ...
      if (metadata) {
        let fragmentId = Utils.getTriplesBySPOG(
          ft[1],
          null,
  ...
  updateHourFragment(newer, old, gat) {
    ...
    let sampleTriple = Utils.getTriplesBySPOG(
      old,
      null,
      'http://datapiloten.be/vocab/timeseries#sampleSize'
    )[0];
    ...
    for (let i in old) {
      if (old[i].predicate === 'http://datapiloten.be/vocab/timeseries#mean') {
        old[i].object =
          '"' +
          this.calculateMean(
            parseInt(newValues.get(old[i].subject)),
            parseInt(Utils.getLiteralValue(old[i].object)),
            sampleValue
          ) +
          '"';
  ...
  createDayFragment(hlevel, gat) {
    ...
    for (let i = 0; i < hlevel.length; i++) {
      ...
      if (
        hlevel[i].predicate === 'http://w3id.org/multidimensional-interface/ontology#hasRangeGate'
      ) {
  ...
  createMonthFragment(dlevel, gat) {
    ...
    let rangeGate =
      this.serverUrl + this.name + '/fragment/' + gat.year() + '_' + (gat.year() + 1) + '/';
    ...
    for (let i = 0; i < dlevel.length; i++) {
      ...
      if (
        dlevel[i].predicate === 'http://w3id.org/multidimensional-interface/ontology#hasRangeGate'
      ) {
  ...
  createYearFragment(mlevel, gat) {
    ...
    mlevel = mlevel.filter(
      m => m.predicate !== 'http://w3id.org/multidimensional-interface/ontology#hasRangeGate'
    );
